...
def run_anomaly_state_extraction(weekly, eventsPerobj_df, objects_attributes, cfg, tables_dir=None):
    tables_dir = Path(tables_dir or cfg.tables_dir)
    tables_dir.mkdir(parents=True, exist_ok=True)

    weekly_closed_anomaly_frame = rolling_iqr_detect(weekly[cfg.closed_event_type_id], **IQR_CONFIG)
    weekly_closed_titles, event_closed_title_join = build_weekly_titles(
        eventsPerobj_df,
        objects_attributes,
        cfg,
        event_type_id=cfg.closed_event_type_id,
    )

    anomaly_context = weekly_closed_anomaly_frame.loc[weekly_closed_anomaly_frame['is_anomaly']].merge(
        weekly_closed_titles,
        on='week_start',
        how='left',
    )
    ...
    if not anomaly_states.empty:
        ...
        assert len(anomaly_states) == int(weekly_closed_anomaly_frame['is_anomaly'].sum()), 'Anomaly row count mismatch.'

    print(f'Detected {len(anomaly_states)} rolling-IQR anomalies for event_type_id={cfg.closed_event_type_id}.')
    print(
        'Joined title attribute rows: '
        f"{event_closed_title_join['attribute_value'].notna().sum()} of {len(event_closed_title_join)} event rows."
    )
    ...
    return weekly_closed_anomaly_frame, anomaly_context, anomaly_states, event_closed_title_join
